...
async function main() {
  ...
  for (let i = 0; i < AGENTS.length; i++) {
    ...
    try {
      ...
      const agentInput: AgentDirectInput = {
        name: agent.name,
        email: agent.email,
        jurisdictions: [],
        endpoint: null,
      ...
      console.log(`✅ Success! TX: ${result.transactionHash}`);
        ...
        const sendResult = await walletClient.sendTokens(
            account.address,
            agent.address,
            coins("1000000000000000000", "unit"),
            TX_FEE
        );
        console.log(i, ": Sent 1 coen to", agent.address, "tx", sendResult.transactionHash)
