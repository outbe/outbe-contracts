...
async function main() {
  ...
  if (argv.fake) {
    entity = {
      ...
      vector_rate: faker.datatype
        .number({ min: 0, max: 1, precision: 0.000000001 })
        .toString(),
      nominal_minor_rate: faker.datatype.number({ min: 0, max: 1000000 }).toString(),
      issuance_minor_rate: faker.datatype.number({ min: 0, max: 1000000 }).toString(),
      symbolic_minor_load: faker.datatype.number({ min: 0, max: 1000000 }).toString(),
      vector_minor_rate: faker.datatype.number({ min: 0, max: 1000000 }).toString(),
      floor_minor_price: faker.datatype.number({ min: 0, max: 1000000 }).toString(),
      state: 'Issued',
  ...
  if (argv.rpcUrl) {
    ...
    const mnemonicPhrase = argv.mnemonic
      ? argv.mnemonic
      : fs.readFileSync(path.resolve(process.cwd(), argv.mnemonicFile), 'utf-8').trim();

    const wallet = await DirectSecp256k1HdWallet.fromMnemonic(mnemonicPhrase, { prefix: argv.prefix });
    const [{ address }] = await wallet.getAccounts();

    const client = await SigningCosmWasmClient.connectWithSigner(argv.rpcUrl, wallet, { prefix: argv.prefix });

    console.log(`Broadcasting transaction to ${argv.rpcUrl}, contract ${argv.contract}`);
    const result = await client.execute(address, argv.contract, msg.submit, 'auto', argv.memo);
