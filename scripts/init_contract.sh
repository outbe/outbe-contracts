...
RESULT=$($binary tx wasm instantiate \
  $code_id "$payload" \
  --label "$label" \
...
RESULT=$($binary query tx --type=hash "$TX_HASH" --node "$RPC" --output json | jq -r "$json_filter")
CONTRACT_ADDRESS=$(echo "$RESULT" | jq -r "$json_filter")
