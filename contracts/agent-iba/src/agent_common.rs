...
use agent_nra::types::ApplicationStatus;
use cosmwasm_std::{Addr, Deps, DepsMut, Env, MessageInfo, Response};
use agent_common::state::{AGENTS, CONFIG};
use agent_common::types::{Agent, AgentInput, AgentStatus};
use agent_nra::msg::{ApplicationResponse, NraAccessResponse};
...
pub fn exec_hold_agent(
    ...
    ensure_active_nra_via_registry(deps.as_ref(), &cfg.agent_registry, info.sender)?;
...
pub fn ensure_active_nra_via_registry(
    deps: Deps,
    agent_registry: &Addr,
    sender: Addr,
    ...
    let resp: NraAccessResponse = deps.querier.query_wasm_smart(
        ...
        &NraQueryMsg::EnsureActiveNra { address: sender },
    )?;
