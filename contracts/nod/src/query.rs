...
use outbe_nft::msg::{ContractInfoResponse, NftInfoResponse, NumTokensResponse, OwnerOfResponse, TokensResponse};
...
mod tests {
    ...
    use crate::msg::{ExecuteMsg, InstantiateMsg, NodCollectionExtension, NodEntity, SubmitExtension};
    ...
    fn test_instantiate_submit_query_and_burn() {
        ...
        // initially no tokens
        let resp: outbe_nft::msg::NumTokensResponse =
            app.wrap().query_wasm_smart(contract_addr.clone(), &QueryMsg::NumTokens {}).unwrap();
        assert_eq!(resp.count, 0);

        // default minter/creator is owner
        let resp: cw_ownable::Ownership<String> =
            app.wrap().query_wasm_smart(contract_addr.clone(), &QueryMsg::GetMinterOwnership {}).unwrap();
        assert_eq!(resp.owner.unwrap(), owner.to_string());
        let resp: cw_ownable::Ownership<String> =
            app.wrap().query_wasm_smart(contract_addr.clone(), &QueryMsg::GetCreatorOwnership {}).unwrap();
        assert_eq!(resp.owner.unwrap(), owner.to_string());
        ...
        app.execute_contract(owner.clone(), contract_addr.clone(), &exec_msg, &[]).unwrap();
        ...
        let resp: outbe_nft::msg::OwnerOfResponse =
            app.wrap().query_wasm_smart(contract_addr.clone(), &QueryMsg::OwnerOf { token_id: token_id.clone() }).unwrap();
        assert_eq!(resp.owner, recipient.to_string());

        // NftInfo should return the correct extension
        let resp: outbe_nft::msg::NftInfoResponse<NodData> =
            app.wrap().query_wasm_smart(contract_addr.clone(), &QueryMsg::NftInfo { token_id: token_id.clone() }).unwrap();
        ...
        assert_eq!(resp.extension.issuance_minor_rate, entity.issuance_minor_rate);
        assert_eq!(resp.extension.symbolic_minor_load, entity.symbolic_minor_load);
        ...
        let resp: outbe_nft::msg::TokensResponse =
            app.wrap().query_wasm_smart(contract_addr.clone(), &QueryMsg::Tokens { owner: recipient.to_string(), start_after: None, limit: None }).unwrap();
        assert_eq!(resp.tokens, vec![token_id.clone()]);

        // AllTokens should include token_id
        let resp: outbe_nft::msg::TokensResponse =
            app.wrap().query_wasm_smart(contract_addr.clone(), &QueryMsg::AllTokens { start_after: None, limit: None }).unwrap();
        assert_eq!(resp.tokens, vec![token_id.clone()]);

        // Burn the token
        let exec_msg = ExecuteMsg::Burn { token_id: token_id.clone() };
        app.execute_contract(owner.clone(), contract_addr.clone(), &exec_msg, &[]).unwrap();
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        QueryMsg::ContractInfo {} =>
            to_json_binary(&outbe_nft::query::query_contract_info::<NodConfig>(deps.storage)?),
        QueryMsg::OwnerOf { token_id } =>
            to_json_binary(&outbe_nft::query::query_owner_of(deps.storage, &env, token_id)?),
        QueryMsg::NumTokens {} =>
        ...
        QueryMsg::NftInfo { token_id } =>
            to_json_binary(&outbe_nft::query::query_nft_info::<NodData>(deps.storage, token_id)?),
        QueryMsg::Tokens { owner, start_after, limit } =>
            to_json_binary(&outbe_nft::query::query_tokens(deps, &env, owner, start_after, limit)?),
        QueryMsg::AllTokens { start_after, limit } =>
            to_json_binary(&outbe_nft::query::query_all_tokens(deps, &env, start_after, limit)?),
    }
}